...
| <a name="input_collaborators"></a> [collaborators](#input\_collaborators) | The teams and users to add as collaborators to the repository | <pre>object({<br>    teams = optional(map(set(string)), {})<br>    users = optional(map(set(string)), {})<br>  })</pre> | `{}` | no |
